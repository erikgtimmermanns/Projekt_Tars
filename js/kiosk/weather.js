import { OPEN_METEO } from '../shared/config.js';

export function getWeatherClass(weatherCode) {
    const code = Number(weatherCode);
    if ([0, 1, 2].includes(code)) return 'weather-sunny';
    if ([3, 45, 48].includes(code)) return 'weather-cloudy';
    if ([51, 53, 55, 56, 57, 61, 63, 65, 80, 81, 82].includes(code)) return 'weather-rain';
    if ([71, 73, 75, 77, 85, 86].includes(code)) return 'weather-snow';
    if ([41, 42, 43, 46].includes(code)) return 'weather-fog';
...
export function getWeatherDescription(code) {
    ...
    switch (weatherCode) {
        case 0: return ['☀️', 'Klarer Himmel'];
        case 1: return ['🌤️', 'Meistens klar'];
        case 2: return ['⛅', 'Teilweise bewölkt'];
        case 3: return ['☁️', 'Bewölkt'];
        case 45:
        case 48: return ['🌫️', 'Nebel'];
        case 51:
        case 53:
        case 55: return ['🌦️', 'Nieselregen'];
        case 56:
        case 57: return ['❄️', 'Gefrierender Nieselregen'];
        case 61:
        case 63:
        case 65: return ['🌧️', 'Regen'];
        case 66:
        case 67: return ['🌧️', 'Starker Regen'];
        case 71:
        case 73:
        case 75:
        case 77: return ['❄️', 'Schnee'];
        case 80:
        case 81:
        case 82: return ['🌧️', 'Regenschauer'];
        case 85:
        case 86: return ['🌨️', 'Schneeschauer'];
        case 95: return ['⛈️', 'Gewitter'];
        case 96:
        case 99: return ['⛈️', 'Gewitter mit Hagel'];
        default: return ['🌤️', 'Wetter wird geladen'];
    }
}

export function updateWeatherAnimation(code) {
    const card = document.getElementById('weather-card');
    const weatherCode = Number(code);
    const cls = getWeatherClass(weatherCode);
    const validClasses = ['weather-default','weather-sunny','weather-cloudy','weather-fog','weather-rain','weather-snow','weather-thunder'];
...
export function renderForecast(data) {
    ...
    if (!forecastHost || !data || !data.daily || !Array.isArray(data.daily.time)) return;
...
export class WeatherAnimator {
    ...
    setMode(mode) {
        ...
        switch (this.targetMode) {
            case 'weather-rain': this.wind = 40; this.targetCounts.rain = base * 3; this.targetCounts.cloud = Math.max(6, Math.round(w / 180)); break;
            case 'weather-snow': this.wind = 8; this.targetCounts.snow = Math.round(base * 1.6); this.targetCounts.cloud = Math.max(4, Math.round(w / 220)); break;
            case 'weather-fog': this.wind = 6; this.targetCounts.fog = Math.max(6, Math.round(w / 120)); this.targetCounts.cloud = Math.max(3, Math.round(w / 300)); break;
            case 'weather-thunder': this.wind = 60; this.targetCounts.rain = base * 2.5; this.targetCounts.cloud = Math.max(8, Math.round(w / 140)); break;
            case 'weather-sunny': this.wind = 6; this.targetCounts.spark = Math.max(8, Math.round(w / 80)); this.targetCounts.cloud = Math.max(2, Math.round(w / 320)); break;
            case 'weather-cloudy': this.wind = 12; this.targetCounts.cloud = Math.max(4, Math.round(w / 220)); this.targetCounts.fog = Math.max(0, Math.round(w / 1200)); break;
            default: this.wind = 6; this.targetCounts.cloud = Math.max(2, Math.round(w / 400));
        }
    ...
    _tick(ts) {
        if (!this.running) return; if (!this._lastTs) this._lastTs = ts; const dt = Math.min(60, ts - this._lastTs) / 1000; this._lastTs = ts; this._update(dt); this._draw(); requestAnimationFrame(this._boundTick);
    }
    ...
    _update(dt) {
        ...
        for (let i = this.particles.length - 1; i >= 0; i--) {
            ...
            if (p.type === 'rain') {
                p.y += p.speed * dt; p.x += this.wind * dt * (0.6 + Math.random() * 0.8);
                if (p.y > h + p.len) { p.y = -10 - Math.random() * 40; p.x = Math.random() * w; }
            } else if (p.type === 'snow') {
                p.y += p.speed * dt; p.x += p.drift * 20 * dt + this.wind * 0.02 * dt; if (p.y > h + 10) { p.y = -10 - Math.random() * 20; p.x = Math.random() * w; }
            } else if (p.type === 'fog') {
                p.x += p.speed * 0.05 * dt + this.wind * 0.02 * dt; if (p.x - p.w > w) p.x = -p.w;
            } else if (p.type === 'spark') {
                p.life -= dt; p.x += p.vx * dt; p.y += p.vy * dt; p.alpha = Math.max(0, p.life / 2);
                if (p.life <= 0 || p.y < -50) { p.x = Math.random() * w; p.y = Math.random() * h * 0.6; p.life = 1 + Math.random() * 2; }
            } else if (p.type === 'cloud') {
                p.x += p.speed * dt * (0.2 + (this.wind / 200)); if (p.x - p.w > w) p.x = -p.w;
            }

            const desired = (this.targetCounts[p.type] || 0) > 0;
            if (!desired) {
                if (typeof p.alpha !== 'number') p.alpha = 1; p.alpha -= dt * 1.4; if (p.alpha <= 0) { this.particles.splice(i, 1); continue; }
            }
    ...
    _draw() {
        ...
        if (this.targetMode === 'weather-thunder' && Math.random() < 0.01) {
            ctx.fillStyle = 'rgba(255,255,255,0.22)'; ctx.fillRect(0, 0, this.width, this.height);
            ctx.strokeStyle = 'rgba(255,255,230,0.95)'; ctx.lineWidth = 2.2; ctx.beginPath(); let sx = Math.random() * this.width * 0.8 + this.width * 0.1; let sy = Math.random() * this.height * 0.4 + 10; ctx.moveTo(sx, sy);
            for (let i = 0; i < 8; i++) { sx += (Math.random() - 0.5) * 60; sy += 20 + Math.random() * 40; ctx.lineTo(sx, sy); }
            ctx.stroke();
        }
        ...
        for (const p of this.particles) {
            ...
            else if (p.type === 'cloud') { ctx.save(); const blurPx = Math.max(0, p.layer * 3); ctx.filter = `blur(${blurPx}px)`; let baseColor; if (this.targetMode === 'weather-cloudy') { baseColor = `rgba(255,255,255,${Math.min(0.95, 0.6 * p.alpha + 0.25)})`; } else if (this.targetMode === 'weather-rain' || this.targetMode === 'weather-thunder') { const a = Math.min(0.9, 0.18 + p.alpha * 0.6); baseColor = `rgba(70,80,95,${a})`; } else { baseColor = `rgba(255,255,255,${p.alpha})`; }
                ctx.fillStyle = baseColor; ctx.beginPath(); ctx.ellipse(p.x, p.y, p.w * 0.6, p.h * 0.5, 0, 0, Math.PI * 2); ctx.fill(); ctx.beginPath(); ctx.ellipse(p.x + p.w * 0.35, p.y - p.h * 0.15, p.w * 0.45, p.h * 0.45, 0, 0, Math.PI * 2); ctx.fill(); ctx.restore(); }
        }
    }
...
export function ensureWeatherAnimator() {
    if (!weatherAnimator) weatherAnimator = new WeatherAnimator('weather-canvas');
...
export async function loadWeather() {
    ...
    try {
        const apiUrl = "https://api.open-meteo.com/v1/forecast" +
            `?latitude=${OPEN_METEO.latitude}` +
            `&longitude=${OPEN_METEO.longitude}` +
            "&current_weather=true" +
            "&hourly=relativehumidity_2m" +
            "&daily=weather_code,temperature_2m_max,temperature_2m_min,precipitation_probability_max" +
            `&timezone=${encodeURIComponent(OPEN_METEO.timezone)}`;
        ...
        let weatherCode = null; let temp = null; let windSpeed = null; let humidityVal = null;
        ...
        if (data.current_weather) {
            ...
            if (data.hourly && Array.isArray(data.hourly.time) && Array.isArray(data.hourly.relativehumidity_2m)) {
                const times = data.hourly.time; const hums = data.hourly.relativehumidity_2m; const idx = times.indexOf(cw.time);
                if (idx >= 0) humidityVal = Number(hums[idx]); else humidityVal = Number(hums[hums.length - 1]);
            }
        ...
        console.log("Wetterdaten erfolgreich geladen:", { weatherCode, weatherClass: getWeatherClass(weatherCode), description: weather[1], temperature: roundedTemperature, humidity: roundedHumidity, wind: roundedWind });
    } catch (error) {
        console.error("Fehler beim Laden der Wetterdaten:", error);
        if (document.getElementById('weather-icon')) document.getElementById('weather-icon').textContent = "⚠️";
        if (document.getElementById('temperature')) document.getElementById('temperature').textContent = "--°C";
        if (document.getElementById('weather-description')) document.getElementById('weather-description').textContent = "Wetterdaten nicht verfügbar";
        if (document.getElementById('humidity')) document.getElementById('humidity').textContent = "--%";
        if (document.getElementById('wind')) document.getElementById('wind').textContent = "-- km/h";
